...
def listar_tiendas(
    ...
    tiendas = (
        ...
        .filter(models.Tienda.activa == True)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router_admin.post(
    "/admin/tiendas",
    response_model=schemas.TiendaOut,
...
def crear_tienda(
    ...
    if tienda.sector not in SECTORES:
        raise HTTPException(
            status_code=400, detail=f"Sector inválido. Opciones: {SECTORES}"
        )
    ...
    if manager_id:
        ...
        manager = (
            db.query(models.Usuario).filter(models.Usuario.id == manager_id).first()
        )
        if manager:
...
def obtener_tienda(
    ...
    if current_user.rol != "admin":
        if current_user.tienda_id != tienda_id:
            raise HTTPException(
                status_code=403, detail="No tienes acceso a esta tienda"
            )
    ...
    if nuevo_manager_id is not None:
        ...
        if nuevo_manager_id:
            manager = (
                ...
                .filter(models.Usuario.id == nuevo_manager_id)
                .first()
            )
            if manager:
...
@router_publico.get(
    "/tiendas/por-subdominio/{subdominio}", response_model=schemas.TiendaPublicaOut
)
def obtener_tienda_por_subdominio(subdominio: str, db: Session = Depends(get_db)):
    tienda = (
        db.query(models.Tienda)
        .filter(models.Tienda.subdominio == subdominio, models.Tienda.activa == True)
        .first()
    )
    if not tienda:
        raise HTTPException(status_code=404, detail="Tienda no encontrada")
...
@router_publico.get("/tiendas/actual", response_model=schemas.TiendaPublicaOut)
def obtener_tienda_actual(
    request: Request,
    subdominio: str = None,
    db: Session = Depends(get_db),
):
    """Obtiene la tienda actual basada en el subdominio del Host (middleware) o query param."""
    sub = getattr(request.state, "subdominio", None) or subdominio
    if not sub:
        raise HTTPException(
            status_code=400,
            detail="Subdominio no detectado. Usa ?subdominio=XXX o accede via subdominio.dominio.com",
        )
    tienda = (
        db.query(models.Tienda)
        .filter(models.Tienda.subdominio == sub, models.Tienda.activa == True)
        .first()
    )
    if not tienda:
        raise HTTPException(status_code=404, detail="Tienda no encontrada")
...
def obtener_tienda_por_id(tienda_id: int, db: Session = Depends(get_db)):
    tienda = (
        db.query(models.Tienda)
        .filter(models.Tienda.id == tienda_id, models.Tienda.activa == True)
        .first()
    )
    if not tienda:
        raise HTTPException(status_code=404, detail="Tienda no encontrada")
...
def obtener_tienda_por_slug(slug: str, db: Session = Depends(get_db)):
    tienda = (
        db.query(models.Tienda)
        .filter(models.Tienda.slug == slug, models.Tienda.activa == True)
        .first()
    )
    if not tienda:
        raise HTTPException(status_code=404, detail="Tienda no encontrada")
...
@router_publico.get(
    "/tiendas/{tienda_id}/productos", response_model=List[schemas.ProductoOut]
)
def listar_productos_tienda(
    tienda_id: int, skip: int = 0, limit: int = 50, db: Session = Depends(get_db)
):
    productos = (
